...
# under a NO  marine protected area (MPA) consisting of fishermen of 
...
import shutil, os
subdir = 'NO_MPA' # subfolder name for plot files
...
def initialize():
    ''' 
     An agent-based model to investigate harvesting behaviour of heterogenous artisanal fishermen under a NO
     marine protected area (MPA) .
    '''
    global time1, agents, fish, fish_data, total_hav_data, current_hav_data, fishermen, fishermen_data1,  fishermen_data2
    time1 = 0. # time
...
def update_fish():
    ...
    fish_ag = rd.sample([j for j in agents if j.type == 'fish'],1)[-1] #fish[randint(len(fish))]#randomly select an fish agent
...
def update_fisherman():
    ...
    fisherman_ag = rd.sample([j for j in agents if j.type == 'fishers'],1)[-1] #fishermen[randint(len(fishermen))]#randomly select an fisherman agent
    ...
    if all([len(fishers_neighbors_harvest) > 0 , fishers_neighbors_harvest[-1][0] > fisherman_ag.harvest]) : # if there exist fisherman in neighbourhood 
        deltax = fishers_neighbors_harvest[-1][-1].x - fisherman_ag.x   #move in the direction of one with greater harvest than focal fisherman 
...
def update_one_unit_time():
    ...
    csvfile = "no_mpa.csv"   # a csv-file output 
    header = [key for key in sorted(current_hav_data)]
    header.append('total_haverst') ; header.append('total_fish')
    main_data = [current_hav_data[key] for key in sorted(current_hav_data)]
    main_data.append(fishermen_data2) ; main_data.append(fish_data) 
