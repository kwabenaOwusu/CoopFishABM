...
# under a DISTANCED-SPACED  marine protected area (MPA) consisting of fishermen of 
...
import shutil, os
subdir = 'DISTANCED_SPACED_MPA' # subfolder name for plot files
...
def initialize():
    ''' 
     An agent-based model to investigate harvesting behaviour of heterogenous artisanal fishermen under a DISTANCED-SPACED
    ...
    for j in range(num_fishers + init_fish):    
        ...
        if ag.type == 'fishers':
            ...
            while True: # randomly assign spatial_position to fishermen 
                ...
                if all([not((-0.9 <= ag.x <= -0.5) and (0.1 <= ag.y <= 0.9)), 
                        not((0.5 <= ag.x <= 0.9) and (-0.9 <= ag.y <= -0.1))]): # keep looping until spatial_position falls outside the MPA
                    break
    ...
    any([((-0.9 <= j.x <= -0.5) and (0.1 <= j.y <= 0.9)), ((0.5 <= j.x <= 0.9) and (-0.9 <= j.y <= -0.1))])])]
...
def observe():
   ...
   # Lines enclosing the MPA
    plt.vlines(-0.9, 0.1, 0.9,lw=2, color='k') # (x1,y1,y2) 
    plt.vlines(-0.5, 0.1, 0.9, lw=2, color='k') # (x2,y1,y2)
    plt.hlines(0.1,-0.9,-0.5, lw=2, color='k') # (y1,x1,x2)
    plt.hlines(0.9, -0.9, -0.5, lw=2, color='k') # (y2, x1,x2)
    
    plt.vlines(0.5, -0.9, -0.1,lw=2, color='k') # (x1,y1,y2) 
    plt.vlines(0.9, -0.9, -0.1, lw=2, color='k') # (x2,y1,y2)
    plt.hlines(-0.9,0.5, 0.9 ,lw=2, color='k') # (y1,x1,x2)
    plt.hlines(-0.1,0.5, 0.9, lw=2, color='k') # (y2, x1,x2)
    
    plt.savefig('abm_%04d.png' % time1, bbox_inches='tight', pad_inches=0 ) # save-figures to frames folder , dpi=1000
    #plt.show()
...
def update_fisherman():
    ...
    fish_neighbors = [nb for nb in agents if nb.type == 'fish' and ((fisherman_ag.x - nb.x)**2 + (fisherman_ag.y - nb.y)**2) < r_sqr 
        and  all([not((-0.9 <= nb.x <= -0.5) and (0.1 <= nb.y <= 0.9)), not((0.5 <= nb.x <= 0.9) and (-0.9 <= nb.y <= -0.1))])] # detecting fishes in neighbourhood
    ...
    if all([len(fishers_neighbors_harvest) > 0 , fishers_neighbors_harvest[-1][0] > fisherman_ag.harvest]) : # if there exist fisherman in neighbourhood 
        ...
        if all([not((-0.9 <= (fisherman_ag.x + move_fishers*math.cos(theta_1)) <= -0.5) and (0.1 <= (fisherman_ag.y + move_fishers*math.sin(theta_1)) <= 0.9)), 
                not((0.5 <= (fisherman_ag.x + move_fishers*math.cos(theta_1) <= 0.9)) and (-0.9 <= (fisherman_ag.y + move_fishers*math.sin(theta_1)) <= -0.1))]):
            ...
            while True: # in the case it can land you on an MPA move in a random direction
                ...
                if all([not((-0.9 <= fisherman_ag.x <= -0.5) and (0.1 <= fisherman_ag.y <= 0.9)), 
                        not((0.5 <= fisherman_ag.x <= 0.9) and (-0.9 <= fisherman_ag.y <= -0.1))]):
                        fisherman_ag.x = -1 if fisherman_ag.x > 1 else  1 if fisherman_ag.x < -1 else fisherman_ag.x
                        fisherman_ag.y = -1 if fisherman_ag.y > 1 else  1 if fisherman_ag.y < -1 else fisherman_ag.y
                        break
       ...
       theta_empt1 = 0 ; theta_empt2 = 0
       while True: # if no fishermen around move randomly
            theta_2 = 2*math.pi*rd.random()
            fisherman_ag.x +=  move_fishers*math.cos(theta_2) - theta_empt1  # move  'move_fishers' step in a random direction
            fisherman_ag.y +=  move_fishers*math.sin(theta_2) - theta_empt2 
            theta_empt1 = move_fishers*math.cos(theta_2) ; theta_empt2 = move_fishers*math.sin(theta_2)
...
def update_one_unit_time():
    ...
    fish_data_MPA.append(sum([1 for j in agents if j.type == 'fish' and 
                        any([((-0.9 <= j.x <= -0.5) and (0.1 <= j.y <= 0.9)), 
                            ((0.5 <= j.x <= 0.9) and (-0.9 <= j.y <= -0.1))])])) # detecting number of fishes in MPA
    ...
    csvfile = "distanced_mpa.csv"   # a csv-file output 
