...
# under different spatial design of marine protected area (MPA) and cooperation levels.
...
n = 150 # total time-period for asimulation 

# Fish characteristics #
growth_prob =  0.2 # intrinsic growth probability 
init_fish = 150 # initial biomass 
move_fish = 0.2 # speed of fish 
rad_repulsion = 0.025 #  radius of repulsion zone centered at fish
rad_orientation = 0.06 # radius of orientation zone centered at fish
rad_attraction =  0.1 #  radius of attraction zone centered at fish
...
move_fishers = 0.35 # speed of pirogue 
q = 0.65 # catchability coefficient
r = 0.2 # neighbourhood radius of a fisherman (length of purse seine)
r_sqr = r ** 2 # radius squared

# Cooperation level (must sum up to  number of fishermen) #
fully_noncoop = 2 # number of fully_noncooperative fishermen
noncoop = 3 # number of noncooperative fishermen
cond_coop = 4 # number of conditional_cooperative fishermen
coop = 5 # number of cooperative fishermen
fully_coop = 6 # number of fully_cooperative fishermen

# Type of simulation #
MPA = 'no'  # run simulation with only an MPA? : no or yes (where a 'yes' implies only with MPA and 'no' implies run only without MPA)
Both = 'yes'  # run simulation partly without MPA and partly with MPA? : no or yes
Time_MPA = 125 # If Both = 'yes', which time to terminate the MPA? 
Type_MPA = 'spaced' # If MPA  = 'yes' or Both = 'yes', which type of MPA? : spaced or single
Dist_MPA = 0.6 # If Type_MPA = 'spaced', What should be the distance between MPA ?
Frac_MPA = 0.4  #What fraction of the fishing environments should be used for MPA

# Coordinates for MPA based on type of simulation required 
...
def initialize():
    ...
    time1 = 0. # time
    agents = []  # empty list to contain biomass and fishermen
    fish_data = [init_fish]  # list to contain biomass
    ...
    for j in range(num_fishers + init_fish):    
        ...
        if ag.type == 'fishers':
            ...
            total_hav_data[ag.num]  = [ag.harvest]  # initialise total catch of fishermen by cooperative trait
    ...
    fishermen_data3 = [fish_data[-1] - fish_data_MPA[-1]] # initialise biomass outside MPA
 #------------------------------------------------------------------------------------------------------------------        
...
def observe():
    ...
    if len(fishermen) > 0:
        ...
        colors = np.linspace(0.1, 1, 5) ; mymap = plt.get_cmap("Reds") ; my_colors = mymap(colors) 
