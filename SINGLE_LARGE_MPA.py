...
# under a SINGLE-LARGE-SPACED  marine protected area (MPA) consisting of fishermen of 
...
import shutil, os
subdir = 'SINGLE_LARGE_MPA' # subfolder name for plot files
...
# Distribution of cooperative-trait (must sum up to number of fishermen (num_fishers))#
...
def initialize():
    ''' 
     An agent-based model to investigate harvesting behaviour of heterogenous artisanal fishermen under a SINGLE-LARGE
    ...
    for j in range(num_fishers + init_fish):    
        ...
        if ag.type == 'fishers':
            ...
            while True: # randomly assign spatial_position to fishermen 
                ...
                if not((-0.4 <= ag.x <= 0.4) and (-0.4 <= ag.y <= 0.4)) : # keep looping until spatial_position falls outside the MPA
                    break
    ...
    fish_data_MPA = [sum([1 for j in agents if j.type == 'fish' and  ((-0.4 <= j.x <= 0.4) and (-0.4 <= j.y <= 0.4))])]
...
def observe():
   ...
   # Lines enclosing the MPA
    # Lines enclosing the MPA
    plt.vlines(-0.4, -0.4, 0.4,lw=2, color='k') # (x1,y1,y2) 
    plt.vlines(0.4, -0.4, 0.4, lw=2, color='k') # (x2,y1,y2)
    plt.hlines(-0.4,-0.4, 0.4, lw=2, color='k') # (y1,x1,x2)
    plt.hlines(0.4, -0.4, 0.4, lw=2, color='k') # (y2, x1,x2)
...
def update_fisherman():
    ...
    fish_neighbors = [nb for nb in agents if nb.type == 'fish' and ((fisherman_ag.x - nb.x)**2 + (fisherman_ag.y - nb.y)**2) < r_sqr 
        and not((-0.4 <= nb.x <= 0.4) and (-0.4 <= nb.y <= 0.4))] # detecting fishes in neighbourhood
    ...
    if all([len(fishers_neighbors_harvest) > 0 , fishers_neighbors_harvest[-1][0] > fisherman_ag.harvest]) : # if there exist fisherman in neighbourhood 
        ...
        if not((-0.4 <= (fisherman_ag.x + move_fishers*math.cos(theta_1)) <= 0.4) and (-0.4 <= (fisherman_ag.y + move_fishers*math.sin(theta_1)) <= 0.4)): 
            ...
            while True: # in the case it can land you on an MPA move in a random direction
                ...
                if not((-0.4 <= fisherman_ag.x <= 0.4) and (-0.4 <= fisherman_ag.y <= 0.4)):
                    fisherman_ag.x = -1 if fisherman_ag.x > 1 else  1 if fisherman_ag.x < -1 else fisherman_ag.x
                    fisherman_ag.y = -1 if fisherman_ag.y > 1 else  1 if fisherman_ag.y < -1 else fisherman_ag.y
                    break
...
def update_one_unit_time():
    ...
    fish_data_MPA.append(sum([1 for j in agents if j.type == 'fish' and ((-0.4 <= j.x <= 0.4) and (-0.4 <= j.y <= 0.4))])) # detecting number of fishes in MPA
    ...
    csvfile = "single_mpa.csv"   # a csv-file output 
